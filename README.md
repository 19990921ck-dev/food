...
### 3.1. 註冊頁面
- **輸入欄位**：
  - `ID NAME` (登入帳號)：提示「只能使用英文，區分大小寫，上限12個字」。
  - `USERNAME` (系統稱呼)：使用者希望系統如何稱呼自己。
  - `PW` (密碼)：長度至少8碼，且必須包含大小寫英文字母及數字。
  - `確認密碼`：驗證兩次密碼輸入是否一致。
- **錯誤提示**：
  - 當 `USERNAME` 已被註冊時，提示「此使用者名稱已被使用，請更換」。
...
| `ID NAME`         | String   | **唯一值**。使用者自訂的**登入帳號**。限制為英文、區分大小寫、上限12個字。                 |
